...
* [AI通识简述](#ai通识简述)
  ...
  * [核心概念](#核心概念)
    ...
    * [大模型（Large Language Model）](#大模型large-language-model-1)
...
- **分类**：
  - **大语言模型（Large Language Model）**：大语言模型是指能够理解和生成人类语言的模型，通常用于文本生成、文本分类、情感分析等任务。
  - **多模态模型（Multimodal Model）**：多模态模型是指能够同时处理多种数据类型的模型，例如文本、图像、视频等。
  - **生成式模型（Generative Model）**：生成式模型是指能够生成新数据的模型，例如文本生成、图像生成、视频生成等。
  - **Agent 模型（Agent Model）**：Agent 模型是指能够执行任务的模型，例如自动回复、自动决策、自动开发等。
- **特点**：
  - **规模和参数量大**：大模型通过其庞大的规模(拥有从数亿到数千亿级 别的参数数量)来捕获复杂的数据模式,使得它们能够理解和生成极其丰富的信息。
  - **广泛数据集的预训练**：大化的数据进行预化的数据进行预训练,以学习 广泛的知识表示,能够掌握语言、图像等数据的通用特征。
  - **计算资源需求大**：巨大的巨大的模型规模带来了高昂的计算和资源需求,包括但不限于数据存储、训练时间、能量消耗和硬件设施。
